...
/**
 * Get ALL messages in a DM channel (no filtering by ts — DMs rarely use real threads,
 * so relying on conversations.replies against a single ts is unreliable for 1:1 DMs).
 * We fetch the full recent history and let the caller decide what's relevant by author.
 */
export async function dmAllHistory(user, channelId, limit = 100) {
  const r = await slackGet(userToken(user), "conversations.history", { channel: channelId, limit: String(limit) });
  if (!r.ok) { console.error("conversations.history error:", r.error, "channel:", channelId); return []; }
  return (r.messages || []).filter(m => m.type === "message" && m.subtype !== "bot_message");
}

/**
 * Returns true thread replies to a specific message ts (rare for DMs, common for channels).
 */
export async function threadReplies(user, channelId, ts) {
  ...
  return (r.messages || []).filter(m => m.ts !== String(ts) && m.subtype !== "bot_message");
}

...
// Find a Slack user by full or partial name — searches all workspace members
export async function lookupByName(user, name) {
  const nameLower = name.toLowerCase().trim();
  let cursor = "";
  // Paginate through all workspace members (max 200 per page)
  for (let page = 0; page < 10; page++) {
    ...
    // Partial match fallback — first name + last name both present
    for (const m of members) {
      const realName = (m.real_name || "").toLowerCase();
      if (realName.includes(nameLower) || nameLower.includes(realName.split(" ")[0])) {
        // Only use partial if it's a strong match (first + last both match)

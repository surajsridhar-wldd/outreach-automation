...
export async function checkOneRecord(rec, owner) {
  ...
  if (!messages.length) {
    ...
    const { error: upErr } = await db.from("outreach_records").update({ status: newStatus, last_action_at: now }).eq("id", rec.id);
  ...
  if (upErr) {
    console.error("checkOneRecord update error:", upErr.message);
    return { newStatus: rec.status, error: `Reply found but DB update failed: ${upErr.message}` };
  }

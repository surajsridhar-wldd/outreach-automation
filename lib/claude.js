...
export async function categorizeIssuesBatch({ items, categories, chunkSize = 12 }) {
  ...
  async function classifyChunk(chunk, depth = 0) {
    ...
    const text = await callHaiku({ system, user, maxTokens: Math.max(2000, chunk.length * 120) });
    ...
    // Parse failed. Don't give up on the whole chunk — split and retry, down to
    // single-item calls if necessary. Only after a single item still fails do we
    // record it as tag:null (a real "couldn't classify" outcome, not a batch fluke).
    if (chunk.length === 1) {
      console.error("categorizeIssuesBatch: single-item classify failed to parse, marking null:", chunk[0].id);

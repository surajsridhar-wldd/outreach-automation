...
async function callHaiku({ system, user, maxTokens = 500 }) {
  ...
  if (!res.ok) {
    // Anthropic error responses look like { type:"error", error:{ type, message } }.
    // Previously this fell through silently — data.content was undefined, producing
    // an empty string that could never parse as JSON, which every caller then
    // treated as "the model returned nothing useful" and fell back to tag:null with
    // confidence:0 — indistinguishable from a genuine judgment. Throw instead so the
    // real cause (bad key, rate limit, invalid model, etc.) surfaces in diagnostics.
    const msg = data?.error?.message || data?.message || `Anthropic API error (HTTP ${res.status})`;
...
function parseJson(text, fallback) {
  ...
  // Model sometimes wraps the JSON in commentary despite instructions, or the
  // response gets cut off. Try to pull out the first well-formed [...] or {...}.
...
//
// IMPORTANT: if a chunk's response fails to parse as JSON (truncated output, stray
// commentary, etc.), we do NOT silently mark every item in it as uncategorized —
// that produced a real bug where a whole batch of unrelated records all got
// tag:null/confidence:0 together, indistinguishable from genuine "doesn't fit any
// category" judgments. Instead we retry a failed chunk at half size (down to
// single items) before giving up on any individual item.
export async function categorizeIssuesBatch({ items, categories, chunkSize = 12 }) {
  const out = {};
  ...
  const system =
    ...
    'Respond with ONLY valid JSON: an array [{"id":"...","tag":"<one tag>"|null,"confidence":0.0-1.0}], one element per input id, same ids. No commentary before or after the array.';

  async function classifyChunk(chunk, depth = 0) {
    const user = `Items:\n${JSON.stringify(chunk.map(it => ({ id: it.id, campaign: it.campaign || "n/a", issue: it.issue })))}\n\nClassify each.`;

    let text;
    try {
      text = await callHaiku({ system, user, maxTokens: Math.max(2000, chunk.length * 120) });
    } catch (e) {
      // A REAL API error (bad key, rate limit, invalid model, etc.) — retrying
      // smaller chunks won't fix this, it'll just fail the same way ~75 times in a
      // row while silently writing tag:null/confidence:0 for everyone, which is
      // exactly the bug that made this undiagnosable before. Fail fast with one
      // clear message instead.
      throw new Error(`Anthropic API call failed: ${e.message}`);
    }
    ...
    // Parse failed but the API call itself succeeded — this is genuinely "the model
    // responded but not in valid JSON," worth retrying at a smaller size. Only after
    // a single item still fails do we record it as tag:null.
    if (chunk.length === 1) {
      console.error("categorizeIssuesBatch: single-item classify returned non-JSON, marking null:", chunk[0].id, JSON.stringify(text).slice(0, 200));
      out[chunk[0].id] = { tag: null, confidence: 0 };
      return;
    }
    const mid = Math.ceil(chunk.length / 2);
    await classifyChunk(chunk.slice(0, mid), depth + 1);
    await classifyChunk(chunk.slice(mid), depth + 1);
  }

  for (let i = 0; i < items.length; i += chunkSize) {
    await classifyChunk(items.slice(i, i + chunkSize));
  }
  return out;
}

...
export default function Settings() {
  ...
  async function runRecategorize(scope) {
    ...
    try {
      let guard = 0, totalTagged = 0, totalChanged = 0, lastDiag = null, hardError = null;
      while (guard++ < 20) {
        ...
        if (!res || res.error) { hardError = (res?.error||"Failed"); lastDiag = res?.diag; break; }
        ...
        const items = res.diag?.itemsWithIssue ?? null;
        if ((res.remaining||0) === 0 || (res.tagged||0) === 0 || items === 0) break;
      }
      ...
      if (hardError) {
        setRecatMsg(`⚠ ${hardError} · ${diagStr(lastDiag)}`);
      } else if (totalChanged === 0) {
        // Nothing actually got categorized — ALWAYS show diagnostics here, no matter
        // which stage was the cause (found nothing, LLM call failed, or every DB
        // write to save the result failed). This was the gap: previously only two
        // specific failure shapes surfaced diagnostics, so a silent DB-write failure
        // fell through to a plain, unhelpful "0 categorized" message.
        setRecatMsg(`⚠ 0 categorized. Diagnostics — ${diagStr(lastDiag)}`);
      } else {
        setRecatMsg(`✓ Done — ${totalChanged} record(s) newly categorized.`);
      }

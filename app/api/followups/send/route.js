...
export async function POST(req) {
  ...
  for (const id of ids) {
    ...
    if (!rec || rec.status !== "no_reply") { results.push({ id, ok: false, error: "not in follow-up queue" }); continue; }
    ...
    if (rec.followups >= MAX_FOLLOWUPS) {
      ...
      results.push({ id, ok: false, error: "max follow-ups reached → marked stalled" });
      continue;
    }
    ...
    try {
      if (rec.channel === "email") {
        ...
        // anchor moves to the follow-up message so loose replies after it are caught
        await db.from("outreach_records").update({ slack_message_ts: sent.ts }).eq("id", id);
      }
      await db.from("outreach_records").update({
        status: "followup", followups: n, last_action_at: new Date().toISOString(),
      }).eq("id", id);
      await logEvent({ outreachId: id, userId: user.id, action: "followup_sent", prevStatus: "no_reply", newStatus: "followup", payload: { n } });
